defmodule DatatransHelperTest do
  ...
  describe "generate_sign1/3" do
    test "map has same output as config" do
      ptest [
        ...
        expected = :sha256
        |> :crypto.hmac(Base.decode16!(sign1_hmac_key),
  ...
  describe "generate_sign1/2" do
    test "map has same output as config" do
      ptest [
        ...
        signature = generate_sign1(%Money{amount: amount, currency: String.to_atom(currency)}, reference)
  ...
  describe "valid_sign1?/2" do
    test "verifies correctly" do
      ptest [
        ...
        assert valid_sign1?(signature, %Money{amount: amount, currency: String.to_atom(currency)}, reference)
        refute valid_sign1?("something", %Money{amount: amount, currency: String.to_atom(currency)}, reference)
      end
    end
  end
  ...
  describe "generate_sign2/3" do
    ptest [
      ...
      sign2_hmac_key: @property_sign2_hmac_key
    ] do
      Application.put_env(:datatrans_helper, :merchant_id, merchant_id)
      ...
      expected = :sha256
      |> :crypto.hmac(Base.decode16!(sign2_hmac_key),
        (if is_integer(merchant_id), do: Integer.to_string(merchant_id), else: merchant_id) <>
        Integer.to_string(amount) <>
        currency <>
  ...
  describe "generate_sign2/2" do
    ptest [
      ...
      signature = generate_sign2(%Money{amount: amount, currency: String.to_atom(currency)}, upp_transaction_id)
  ...
  describe "valid_sign2?/2" do
    test "verifies correctly" do
      ptest [
        ...
        assert valid_sign2?(signature, %Money{amount: amount, currency: String.to_atom(currency)}, upp_transaction_id)
        refute valid_sign2?("something", %Money{amount: amount, currency: String.to_atom(currency)}, upp_transaction_id)
      end
    end
  end
end
